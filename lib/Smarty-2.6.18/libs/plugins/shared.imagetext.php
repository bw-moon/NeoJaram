...
function smarty_imagetext($params)
	...
	### Preferences
	// Read config file
    if (empty($params['textcache'])) $textcache_root = 'textcache';
	...
	$config['main']['cache']	= "$textcache_root/cache/"; // Nur dieser Ordner muss öffentlich zugänglich sein
	...
	if (file_exists($imgurl) && $style['dev'] != true && $style['dynamic'] != true) 
		  ...
		  if ( empty($style['html']) ) return $style['prehtml'].'<img src="'.$imgurl.'" border="0" alt="'.preg_replace("=\r\n|\r|\n|\t=", ' ', htmlspecialchars($params['text'])).'" '.$style['html'].' />'.$style['posthtml'];
          else return $style['prehtml'].'<img src="'.$imgurl.'" border="0" '.$style['html'].' />'.$style['posthtml'];
    ...
    if ($style['urlonly'] == 'true') return $imgurl;
		...
		if ( empty($style['html']) ) return $style['prehtml'].'<img src="'.$imgurl.'" style="border: '.$border.'px solid #00ff00" alt="'.preg_replace("=\r\n|\r|\n|\t=", ' ', htmlspecialchars($params['text'])).'" '.$style['html'].' />'.$style['posthtml'];
        else return $style['prehtml'].'<img src="'.$imgurl.'" style="border: '.$border.'px solid #00ff00" '.$style['html'].' />'.$style['posthtml'];
	}
?>
