...
<tr>
	...
	<table width="100%" border="0" cellspacing="1" cellpadding="3" class="text">
		<tr>
			<td colspan="2"><h3>Personal Infomation</h3></td></tr>
		<tr>
			<td colspan="2"  style="background-image('/images/div_grad.gif')" ><img src="/images/t.gif" alt="" border="0" width="1" height="4"/></td>
		</tr>
		<tr>
			<td>사용자 번호:</td>
			<td><b><?=$rows['uid']?></b></td>
		...
		<tr>
			<td>사용자 아이디:</td>
			<td><b><?=$rows['user_id']?></b></td>
		</tr>
		<tr>
			<td>이름:</td>
			<td><b><?=$rows['user_name']?></b></td>
		</tr>
		<tr>
			<td>지금 하고 있는 일:</td>
		...
		<tr>
			<td>이메일:</td>
			<td><?=show_email($rows['uid'],$rows['user_email'])?></td>
		</tr>
		<tr>
			<td>홈페이지(블로그):</td>
			<td><a href="http://<?=show_homepage($rows['user_homepage'])?>" target="_blank" class="sub"><?=show_homepage($rows['user_homepage'])?></a></td>
		</tr>
		<? if (isset($_SESSION['jaram_user_uid'])) { ?>
		<tr>
			<td>휴대폰:</td>
			<td><b><?=$rows['user_phone1']?></b></td>
		</tr>
		<tr>
			<td>대표메신저:</td>
			<td><b><?=trim(eregi_replace("@", " at ", $rows['user_msn']));?></b>
			</td>
		</tr>
		<? } ?>
		<tr bgcolor="#ffffff" align="left">
			...
			<p>
			<a href="<?=WEB_ABS_PATH?>/?page=sendmessage&touser=<?=$rows['uid']?>" class="a12b"><?=$rows['user_name']?>에게 이메일 보내기</a>
			</p>
			<p>
