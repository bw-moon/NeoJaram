...
class SchedulerWidget extends CommonWidget {

	public function __construct($pref = array()) {
        $this->widget_name = 'Scheduler';
        $this->widget_desc = '자람 일정을 보여주는 위젯';
        $this->widget_icon = 'layout';
        $this->widget_ver = '1.0';
        $this->widget_nickname = 'scheduler';
        $this->widget_content = '';
        
        parent::__construct($pref);
	}

	private function show_short_plan($PLAN, $state)
	{
		global $show_priod;
	...
	public function getContent() {
		...
		$result_html .= "<table border=\"0\" cellpadding=\"1\" cellspacing=\"0\" width=\"100%\" class=\"thm8\"><tr>";// 년 - 월 출력 

		for($i=0;$i<7;$i++){ // 요일명 출력 
			if($i==0) $result_html .= "<td align=\"center\"><font color=\"red\"><b>$dayname[$i]</b></font></td>\n"; 
			else if($i==6) $result_html .= "<td align=\"center\"><font color=\"blue\"><b>$dayname[$i]</b></font></td>\n"; 
			else $result_html .= "<td align=\"center\"><b>$dayname[$i]</b></td>\n"; 
		}

		$result_html .= "</tr>\n<tr>"; 

		// 처음부터 시작 요일값까진 공백처리 
		for($i=1;$i<=$sday;$i++) $result_html .= "<td height=\"10\">&nbsp;</td>\n"; 
		...
		for($i=1;$i<=$lastday[$mm-1];$i++) { // 1부터 해당 월의 마지막 날까지 반복 
			...
			if (isset($MINI_DATA[$schedule_start]) && is_array($MINI_DATA[$schedule_start]))
			{
			...
			if($chk==1) $result_html .= "<td align=\"center\" valign=\"top\" height=\"10\"".$today."><font color=\"red\">".$day."</font></td>\n"; 
			else if(!$chk) $result_html .= "<td align=\"center\" valign=\"top\" height=\"10\"".$today."><font color=\"blue\">".$day."</font></td>\n";
			else $result_html .= "<td align=\"center\" valign=\"top\" height=\"10\"".$today.">".$day."</td>\n"; 
		...
		{
			if (isset($MINI_DATA[$i]) && is_array($MINI_DATA[$i]))
			{
				for ($j = 0; $j < $MINI_EACH_DAY_PLAN[$i]; $j++)
				{
					$result_html .=  show_short_plan($MINI_DATA[$i][$j], $i);
				}
			}
		}
		return $result_html;
